...
const WeatherScreen: React.FC<WeatherScreenProps> = ({ navigateTo, theme, toggleTheme }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-black">
      ...
      <div className="flex-grow bg-gray-50 dark:bg-gray-900 rounded-t-[40px] p-6 overflow-y-auto">
        {disasterAlert && (
          <div className="bg-green-500 dark:bg-red-500 border-l-4 border-green-700 dark:border-red-700 text-white p-4 rounded-lg mb-6 shadow-lg animate-pulse-bg animate-fadeIn">
            <div className="flex">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 text-center shadow-md mb-6 animate-fadeInUp" style={{animationDelay: '100ms'}}>
            <p className="text-gray-500 dark:text-gray-400">Pune, India</p>
            <div className="flex items-center justify-center my-4">
                <div className="text-yellow-400 mr-4"><SunIcon large /></div>
                <h1 className="text-6xl font-bold text-gray-800 dark:text-white">28°C</h1>
            </div>
            <p className="font-semibold text-gray-700 dark:text-gray-300">Partly Cloudy</p>
            <p className="text-sm text-gray-500 dark:text-gray-400">Feels like 30°C</p>
        </div>

        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-md animate-fadeInUp" style={{animationDelay: '200ms'}}>
            <h3 className="font-bold text-gray-800 dark:text-white mb-4">Upcoming Forecast</h3>
            <div className="flex justify-between overflow-x-auto -mx-6 px-6">
                {forecast.map((item, index) => (
                    <div key={index} className="flex flex-col items-center space-y-2 flex-shrink-0 px-2">
                        <p className="text-sm font-semibold text-gray-600 dark:text-gray-400">{item.time}</p>
                        <div className={`text-2xl ${item.time === '8 PM' ? 'text-gray-500' : 'text-yellow-400'}`}>{item.icon}</div>
                        <p className="font-bold text-gray-800 dark:text-white">{item.temp}°</p>
                    </div>
                ))}
            </div>
        </div>
