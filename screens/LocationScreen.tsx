...
const LocationScreen: React.FC<LocationScreenProps> = ({ navigateTo, theme, toggleTheme, location, locationError, locationLoading, userLocation }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-gradient-page">
      <Header title="My Location" onBack={() => navigateTo(Page.Home)} theme={theme} toggleTheme={toggleTheme} page={Page.Location}/>
      ...
      <div className="bg-light-surface/80 dark:bg-dark-surface/80 backdrop-blur-md p-4 border-t border-light-border dark:border-dark-border transition-colors duration-500">
        <div className="text-center mb-4">
            <h3 className="font-bold text-lg text-light-text dark:text-dark-text">{location ? "Your Current Location" : "Default Location"}</h3>
            <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary">Lat: {userLocation.lat.toFixed(4)}, Lng: {userLocation.lng.toFixed(4)}</p>
        </div>
        <button
